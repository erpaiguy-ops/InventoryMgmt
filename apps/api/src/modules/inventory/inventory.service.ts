...
export class InventoryService {
  ...
  async adjustStock(
    ...
    return this.updateStock(dto.productId, dto.adjustment, 'adjustment' as StockMovementType, {
  ...
  async getLowStockItems(threshold?: number): Promise<(Inventory & { reorderLevel: number })[]> {
    const [
      { data: inventoryRows, error: inventoryError },
      { data: productRows, error: productError },
    ] = await Promise.all([
      this.supabaseService.getTable('inventory').select('*'),
      this.supabaseService.getTable('products').select('id, reorder_level'),
    ]);
  ...
  async validateStock(dto: ValidateStockDto): Promise<{
    available: boolean;
    currentQuantity: number;
    requestedQuantity: number;
  }> {
    const inventory = await this.getInventory(dto.productId).catch(() => null);
  ...
  async reserveStock(
    ...
    const { available, currentQuantity } = await this.validateStock({ productId, quantity });
  ...
  async releaseReservedStock(
    ...
    void quantity;
    await this.getInventory(productId);
