...
export class SuppliersService {
  ...
  async findAll(query: ListSuppliersDto): Promise<PaginatedResult<Supplier>> {
    ...
    let builder = this.supabaseService.getTable('suppliers').select('*', { count: 'exact' });
  ...
  async update(id: string, dto: UpdateSupplierDto): Promise<Supplier> {
    ...
    if (error || !data) {
      throw new NotFoundException(error?.message ?? 'Failed to update supplier');
    }
  ...
  async remove(id: string): Promise<void> {
    ...
    const { error } = await this.supabaseService.getTable('suppliers').delete().eq('id', id);
